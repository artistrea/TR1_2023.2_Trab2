...
export const modulationSchema = z.enum(["QAM8", "ASK", "FSK"]);

export type ModulationType = z.infer<typeof modulationSchema>;

function sine(freq:number = 1, fase:number = 0, precision:number = 300) {
    const xt = [];
    const yt = [];
...
export function modulation(chartBits:string, chartType:ModulationType) {
    const { xt, yt } = sine();
